...
# Options include: whisper-1, gpt-4o-mini-transcribe, gpt-4o-transcribe, etc.
WHISPER_MODEL = os.getenv("OPENAI_STT_MODEL", "whisper-1").strip()
...
def _validate_sample_rate(sr: Any) -> int:
    """
    Avoids wave.Error: bad frame rate / sampling rate not specified.
    """
    try:
        sri = int(sr)
    except Exception:
        raise ValueError(f"Invalid sample rate (not int-like): {sr!r}")

    # Reasonable guardrails for typical audio
    if sri < 8000 or sri > 96000:
        raise ValueError(f"Invalid sample rate (out of range): {sri}")
    return sri


def audio_to_wav_bytes(audio: tuple[int, NDArray[np.int16 | np.float32]]) -> bytes:
    sr, data = audio
    sri = _validate_sample_rate(sr)
...
def transcribe_with_whisper(audio: tuple[int, NDArray[np.int16 | np.float32]]) -> str:
    try:
        ...
        logger.error(f"[whisper] audio_to_wav_bytes failed: {e!r}")
        return ""

    t0 = time.time()

    f = io.BytesIO(wav_bytes)
    # Some clients like having a name for multipart form file
    f.name = "audio.wav"
    ...
    try:
        ...
        logger.error(f"[whisper] OpenAI transcription failed: {e!r}")
        return ""
...
def echo(audio, *args, **kwargs: Any):
    ...
    if looks_like_reset_attempt(transcript):
        ...
        msg_pl = "Jeśli chcesz wyczyścić rozmowę, powiedz dokładnie: „wyczyść rozmowę”."
    ...
    RAW_LOGSAW_LOGS = RAW_LOGS  # keep original name usage stable
...
def create_stream():
    return Stream(
        ...
        mode="send-receive",
        ui_args={
            "title": "PL/EN: OpenAI STT + OpenAI Chat + Kokoro EN + Eleven PL (HARD LOCK EN on remote TTS failure)",
            "inputs": [
