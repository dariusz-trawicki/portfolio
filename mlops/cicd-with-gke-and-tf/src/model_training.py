...
class ModelTraining:
    ...
    def evaluate_model(self,X_test,y_test):
        try:
            ...
            # Accuracy = procent poprawnych y_pred (od 0 do 1):
            accuracy = accuracy_score(y_test,y_pred)
            # average="weighted" znaczy:
            # liczysz precision dla każdej klasy osobno, potem robisz średnią,
            # ale ważysz ją liczbą próbek w klasie (klasy większe mają większy wpływ).
            # Czyli: wynik uwzględnia nierówne liczby klas.
            precision = precision_score(y_test,y_pred ,average="weighted") # od  0 do 1
            recall = recall_score(y_test,y_pred,average="weighted")
            # f1 = 2 * (precision * recall) / (precision + recall)
            # Policz jedną liczbę, która pokazuje, jak dobrze model balansuje między 
            # trafnością a kompletnością, z uwzględnieniem wielkości klas:
